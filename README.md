...
两者都**不连接任何后端**，数据全部在浏览器里模拟，也都不需要部署服务器。
区别只有一个：**要不要让看的人动手**。

| | 交互式演示 | 只读预览 |
| --- | --- | --- |
| 命令 | `npm run demo` | `npm run preview` |
| 能做什么 | 注册、标记进度、做自测、看判分与解析 | 浏览每个页面、筛选资源、翻看题目 |
| 不能做什么 | —— | 标记进度、提交自测、注册全部禁用 |
| 需要登录吗 | 需要（邮箱密码已预填） | **不需要**，自动以预览身份进入 |
| 数据 | 从空状态开始，进度存 localStorage，刷新不丢 | 预置的「学了一半」示例数据，不落盘 |

**只读预览**适合「把界面给别人的看」——课程与题目都是真实的 66 个 / 36 道，
但访问者只能看，且每个访客看到的都是同一份干净的示例数据。页面右下角的浮标会说明这一点。

**交互式演示**适合「让人真的走一遍流程再评估」。

两者都保证「演的是真的」，而不是另摆一套假界面：

- **同一套领域逻辑** —— 进度计算与判卷直接复用 `@aifs/shared`，百分比、解锁状态、通过线都和真实产品一致
- **同一批课程数据** —— 66 个课程与 36 道题由 `npm run demo:data` 从后端种子数据导出

想要可以直接分享或托管的静态版本（产物用相对路径 + HashRouter，双击 index.html 就能打开）：

```bash
npm run build:demo      # → apps/web/dist-demo
npm run build:preview   # → apps/web/dist-preview
```
...
npm run demo         # 交互式演示：不起后端，数据在浏览器里模拟，可标记进度/做自测
npm run preview      # 只读预览：不起后端、无需登录，所有写操作禁用
npm run demo:data    # 从后端种子数据重新导出演示数据（改了课程后重跑）
npm run build:demo   # 构建静态交互演示版到 apps/web/dist-demo
npm run build:preview # 构建静态只读预览版到 apps/web/dist-preview
